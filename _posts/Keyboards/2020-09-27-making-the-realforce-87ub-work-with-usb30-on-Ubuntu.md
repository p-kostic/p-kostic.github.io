---
title: Making the Realforce 87UB work with USB 3.0 on Ubuntu
...
## Problem behavior
Your Realforce 87u or any other (older) Topre keyboard does not work under Ubunu / Linux, but does work under Windows, Grub, or the BIOS on that specific machine.

`$ dmesg | grep usb` will probably give you something like this, where it will try to read the configuration a bunch of time for your port (in my case `usb 1-2`) and give up.
...
There seems to be an issue with my laptop, since the ports are USB 3.0, while the keyboards only works under USB 2.0. Users have reported that the keyboard will work when using a USB 2.0 hub.
...
* `sudo` open `/etc/default/grub` with your favorite editor (e.g. `vim`)
